module ReportingHelper
  ...
  def self.user_report_per_state(state)
    return Users.where(state_id: state)
  end

  def self.states_with_users
    State.select("states.state_name AS name, count(users.id) AS count").joins(:users).group("states.id").order("count DESC").map{ |s| [s.name, s.count] }
  end
end
