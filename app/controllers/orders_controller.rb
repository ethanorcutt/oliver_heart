class OrdersController < ApplicationController
  ...
  def create
    ...
    if charge.outcome.network_status.include? "approved"
      ...
      @user = current_user
      @order = Order.create(user: @user, total: get_cart_total, stripe_key: charge.id, zip: params[:order][:zip], state_id: params[:order][:state_id], address: params[:order][:address], city: params[:order][:city], notes: params[:order][:notes], first_name: params[:order][:first_name], last_name: params[:order][:last_name], address_two: params[:order][:address_two])
  ...
  def update
    ...
    params[:order_lines].each do |order_c_params|
      ...
      logger.warn("Old status id: #{OrderLine.find(order_c_params).order_line_status_id}")
      logger.warn("New order status id: #{order_status}")
      logger.warn("Fulfulled: #{fulfilled}")
